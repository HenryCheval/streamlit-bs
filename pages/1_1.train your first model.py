...
- **Loss**: 损失，是衡量模型预测结果与真实标签之间差距的一个量化指标。在训练过程中，模型试图通过优化算法（如Adam）最小化这个损失值。
- **Accuracy**: 准确率，用于衡量模型分类正确的样本占总样本的比例。
- **Val Loss**: 验证损失，这是模型在验证集上计算得到的损失值。
- **Val Acc**: 验证准确率(Validation Accuracy)，表示模型在验证集上的分类准确率。

如果训练损失和验证损失都很高，说明模型可能欠拟合，模型容量不足以捕捉数据中的模式。如果训练损失很低但验证损失很高，说明模型可能过拟合，模型在训练数据上表现很好，但在新数据上表现不好。
"""
